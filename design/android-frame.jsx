...
function AndroidAppBar({ title = 'Title', large = false }) {
  const menuIcon = (
    <div onClick={() => window.dispatchEvent(new CustomEvent('cph-mobile-menu'))} style={{
      width: 48, height: 48, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
    }}>
      <svg width="24" height="24" viewBox="0 0 24 24">
        <circle cx="12" cy="5" r="2" fill={MD_C.onSurface}/>
        <circle cx="12" cy="12" r="2" fill={MD_C.onSurface}/>
        <circle cx="12" cy="19" r="2" fill={MD_C.onSurface}/>
      </svg>
    </div>
  );
  const backIcon = (
    <div onClick={() => window.dispatchEvent(new CustomEvent('cph-mobile-back'))} style={{
      width: 48, height: 48, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
    }}>
      <svg width="24" height="24" viewBox="0 0 24 24">
        <path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z" fill={MD_C.onSurface}/>
      </svg>
    </div>
  );
  return (
