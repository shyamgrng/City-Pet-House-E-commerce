...
export default function ProductRail({
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        <div className={`mx-4 md:mx-8 ${padBottom} p-[22px] border border-dashed border-[#E4E9EC] rounded-[10px] text-center text-xs text-[#8A96A3]`}>
          No {category.toLowerCase()} products yet — add some in Admin → Shop.
        </div>
      ) : (
        <div className={`grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 px-4 md:px-8 ${padBottom}`}>
          {items.map((p) => (
