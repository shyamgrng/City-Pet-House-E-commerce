...
import { useMicrochip } from "@/context/MicrochipContext";

export default function AdminMicrochipPagePage() {
  const { content, setBannerTitle, setBannerSubtitle, setSearchCaption, updateSection, addFaq, updateFaq, removeFaq } = useMicrochip();
  ...
  return (
    <div className="max-w-[640px]">
      <Link href="/admin/pages" className="text-xs font-semibold text-[#7A56C8] cursor-pointer mb-2.5 inline-block">
        ← Pages
      </Link>
      <div className="font-heading font-bold text-[19px] text-[#1A2027] mb-1.5">Microchipping Archive</div>
      <div className="text-xs text-[#5B6773] mb-[18px]">Editing the content shown on the public Microchipping Archive page.</div>
      ...
      <div onClick={addFaq} className="text-xs font-semibold text-[#7A56C8] cursor-pointer mb-[18px]">
        + Add FAQ
      </div>

      <button onClick={update} className="bg-[#7A56C8] text-white text-center px-5 py-3 rounded-[9px] text-sm font-semibold cursor-pointer max-w-[180px]">
        Update
      </button>
