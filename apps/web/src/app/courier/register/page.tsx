...
export default function CourierRegisterPage() {
  const { submitRegistration } = useCourierRegistration();
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#F7F9FA]">
        <div className="w-[340px] bg-white border border-[#E4E9EC] rounded-2xl p-[30px] text-center">
          ...
          <div className="text-xs text-[#5B6773] leading-relaxed mb-5">
            Thanks, {companyName}! Our admin team will review your details and documents. You&apos;ll get an email with your Courier ID and
            password once you&apos;re approved.
          </div>
          <Link href="/courier/login" className="block w-full bg-primary text-white text-center py-2.5 rounded-lg text-[13px] font-bold">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F7F9FA] py-10 px-4">
      <div className="w-[380px] bg-white border border-[#E4E9EC] rounded-2xl p-[30px]">
        <Link href="/courier/login" className="text-[11px] text-primary font-semibold mb-3 inline-block">
          ← Back to Courier Sign In
        </Link>
        ...
        <RegField label="Company Name" required value={companyName} onChange={setCompanyName} placeholder="e.g. Valley Express Logistics" />
        ...
        <RegField label="Address" required value={address} onChange={setAddress} placeholder="e.g. Kalanki, Kathmandu" mb="mb-4" />
