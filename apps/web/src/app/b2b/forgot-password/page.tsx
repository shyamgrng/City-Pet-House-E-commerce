...
export default function B2BForgotPasswordPage() {
  ...
  const { requestPasswordReset, resetPassword } = useB2BAuth();
  ...
  const submitRequest = () => {
    setError("");
    if (!b2bId.trim()) {
      setError("Please enter your B2B ID.");
      return;
    }
    const res = requestPasswordReset(b2bId.trim());
    if (!res.ok) {
      setError(res.error);
      return;
    }
  ...
  const submitReset = () => {
    ...
    const res = resetPassword(b2bId.trim(), code.trim(), newPassword);
    if (!res.ok) {
      setError(res.error);
      return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#F7F9FA]">
      <div className="w-[340px] bg-white border border-[#E4E9EC] rounded-2xl p-[30px]">
        ...
        <div className="text-xs text-[#8A96A3] mb-[22px]">
          {step === "request" ? "Enter your B2B ID and we'll email you a reset code." : "Enter the code we emailed you and choose a new password."}
        </div>

        {step === "request" && (
          <>
            <Label>B2B ID</Label>
            <Input value={b2bId} onChange={setB2bId} placeholder="e.g. B2B-1001" />
          </>
        )}
        ...
        <Link href="/b2b/login" className="text-xs text-primary font-semibold block text-center">
          ← Back to Sign In
        </Link>
      </div>
    </div>
