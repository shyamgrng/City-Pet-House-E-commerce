...
export default function DoctorForgotPasswordPage() {
  ...
  const { requestPasswordReset, resetPassword } = useDoctorAuth();
  ...
  const submitRequest = () => {
    ...
    const res = requestPasswordReset(doctorId.trim());
    if (!res.ok) {
      setError(res.error);
      return;
    }
  ...
  const submitReset = () => {
    ...
    const res = resetPassword(doctorId.trim(), code.trim(), newPassword);
    if (!res.ok) {
      setError(res.error);
      return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#F7F9FA]">
      <div className="w-[340px] bg-white border border-[#E4E9EC] rounded-2xl p-[30px]">
        ...
        <div className="text-xs text-[#8A96A3] mb-[22px]">
          {step === "request" ? "Enter your Doctor ID and we'll email you a reset code." : "Enter the code we emailed you and choose a new password."}
        </div>

        {step === "request" && (
          <>
            <Label>Doctor ID</Label>
            <Input value={doctorId} onChange={setDoctorId} placeholder="e.g. DR-1042" />
          </>
        )}
        ...
        <Link href="/doctor/login" className="text-xs text-primary font-semibold block text-center">
          ← Back to Sign In
        </Link>
      </div>
    </div>
